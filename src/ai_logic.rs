...
use crate::config::{MEDIUM_AI_DEPTH, HARD_AI_DEPTH, EASY_AI_RANDOM_CHANCE};
...
fn get_best_move(board: &[[CellState; 3]; 3], player: Player, depth: i32) -> Option<BoardMove> {
  ...
  for mov in find_empty_cells(board) {
    ...
    let move_value = minimax(&mut board_copy, depth - 1, false, i32::MIN, i32::MAX, player);
...
pub fn get_medium_ai_move(board: &[[CellState; 3]; 3], player: Player) -> Option<BoardMove> {
  get_best_move(board, player, MEDIUM_AI_DEPTH)
    .or_else(|| get_random_ai_move(board))
}

pub fn get_hard_ai_move(board: &[[CellState; 3]; 3], player: Player) -> Option<BoardMove> {
  get_best_move(board, player, HARD_AI_DEPTH)
    .or_else(|| get_random_ai_move(board))
}
